                                    ...
                                    <div class="i-card-sm card--dark shadow-none rounded-3">
                                        <div class="row justify-content-between align-items-center g-lg-2 g-1">
                                            ...
                                            <div class="col-lg-5 col-md-5 col-sm-5">
                                                <img src="{{ displayImage($gateway->file) }}" class="mxw-80" alt="Vector" style="width:80px; height:43px;">
                                            </div>
            ...
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('user.payment.process') }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="amount" class="col-form-label">Currency Details</label>
                            <div class="input-group mb-3">
                                <select class="form-control" onchange="currency_change(this.value)">
                                    <option value="">Please Select</option>
                                    @foreach($currency_details as $currency_detail)
                                    @php
                                        // Extract only the numeric part
                                        preg_match('/[\d.]+/', $currency_detail->currency, $matches);
                                        $currencyValue = isset($matches[0]) ? (float) $matches[0] : 0;

                                        // Remove unnecessary trailing zeroes (but keep decimals if present)
                                        $formattedValue = rtrim(rtrim(number_format($currencyValue, 2, '.', ''), '0'), '.');
                                    @endphp

                                    <option value="{{ $formattedValue }}">
                                        {{ $currency_detail->add_currency_name }} = {{ $currency_detail->currency }}
                                    </option>
                             
                                    @endforeach
                                </select>
                                <span style="color: #FFF;">
                                    Total= <span id="get_total_currency" style="color: #FFF;"></span>
                                </span>
                            </div>
                        </div>
...
@push('script-push')
    <script>
        "use strict";
        function currency_change(get_val){
            console.log($("#amount").val());
            var get_value = parseFloat($("#amount").val())*parseFloat(get_val);
            // alert("blabla"+get_value);
            $("#get_total_currency").html(get_value);
        }
