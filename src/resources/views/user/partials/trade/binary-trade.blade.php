...
<div class="market-widget mb-4">
    <form method="POST" action="{{ route('user.trade.store', $crypto->id) }}">
        ...
        <div class="d-flex justify-content-center align-items-center gap-3 mt-2">
            {{-- <button type="button" name="algo_switch" class="btn btn-primary capsuled">Algo Switch</button> --}}
            Disable
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="switchButton" @if($user_details->algo_status==1) checked @endif>
                <label class="form-check-label" for="switchButton">Enable Algo Switch</label>
            </div>
        </div>
    </form>
</div>
