...
class InvestmentController extends Controller
{
    ...
    public function showLiverage(Request $request){
        $liverage_details= Liverage::all();
        $setTitle = "Liverage Details";
        $isActiveLiverage = LiverageRequest::where("user_id", Auth::id())->where("status", "1")->first();
        $duration = "";
        if($isActiveLiverage){
            $get_time = Liverage::where("id", $isActiveLiverage->liverage_id)->first();
            $duration = Carbon::parse($isActiveLiverage->updated_at)->addDays($get_time->time_duration);
        }
        $total_algo = LiverageWallet::where("user_id", Auth::id())->sum("wallet");
        return view("user.liverage.show", compact('liverage_details', 'setTitle', 'isActiveLiverage', 'duration', 'total_algo'));
    }

    public function purchaseLiverage($liverage_id, Request $request){
        //get liverage details
        $details= Liverage::where("id", $liverage_id)->first();

        $status= ($details->liverage_setting=="0")?1:0;
        $insert  = LiverageRequest::create([
            "user_id"       =>  Auth::id(),
