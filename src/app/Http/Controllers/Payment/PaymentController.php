...
class PaymentController extends Controller
{
    ...
    public function index(): View
    {
        ...
        // $crypto_payment= CryptoPayment::where("status", "1")->get();
        $currency_details= Add_currency::all();
    ...
    public function success(Request $request): RedirectResponse
    {
        ...
        Log::info('--------- Payment Pay Stack Start --------------', [$request->query('trx')]);
        ...
        if($request->query('gateway_code') == GatewayCode::PAY_STACK->value){
            Log::info('--------- Payment Pay Stack Success --------------', [$request->query('trx')]);
            $trxId = $request->query('trx');
        }
    ...
    public function traditional(Request $request): RedirectResponse
    {
        ...
        $deposit->image= $request->hasFile('image') ? $this->move($request->file('image')) : '';
        // echo "<pre>";
