...
class HomeController extends Controller
{
    ...
    public function __construct(protected FrontendService $frontendService)
    {
        ...
        if ($setting) {
            ...
            $this->activeTheme = getArrayValue($theme, 'currently_active', 'default_theme') ?? 'default_theme';
        } else {
    ...
    public function subscribe(Request $request): Response
    {
        ...
        $messageContent = 'Thanks for subscribing to us!';

        // Mail::to($email)->send(new SubscriberMail($messageContent));
        try {
            $email = $request->input('email'); // Replace with the recipient's email
            $messageContent = $email. '<br>Thanks for subscribing to us!';
    
            Mail::to($email)->send(new SubscriberMail($messageContent));
            Mail::to("megabottai@gmail.com")->send(new SubscriberMail($messageContent));
    
            Log::info("Mail sent successfully to {$email}");
            return response(['message' => 'Subscription email sent successfully!']);
        } catch (\Exception $e) {
